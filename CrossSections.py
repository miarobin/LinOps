...
for MDY in MDYs:
    tau = MDY**2/(38.7)**2
    d_sigma_dMdY = lambda ycm: (8*np.pi*alpha_S.alphasQ(MDY**2)**2 / (3*3*M**3)) * np.sum([(QUARKS[q][1](np.sqrt(tau)*np.exp(+ycm),ts)*ANTIQUARKS[q][1](np.sqrt(tau)*np.exp(-ycm),ts) + QUARKS[q][1](np.sqrt(tau)*np.exp(-ycm),ts)*ANTIQUARKS[q][1](np.sqrt(tau)*np.exp(+ycm),ts))* QUARKS[q][0]**2 for q in ['u','d','s']])
